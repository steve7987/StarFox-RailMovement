...
namespace Assets.Scripts.MapGenerator.Generators
{
    public class TexturesGenerator : MonoBehaviour, IGenerator
    {
        ...
        public void Generate()
        {
            ...
            TerrainLayer[] splatPrototypes = new TerrainLayer[textures.Count];

            Debug.Log("Textures count: " + textures.Count);

            for (int i = 0; i < textures.Count; i++)
            {
                splatPrototypes[i] = new TerrainLayer() { diffuseTexture = (Texture2D)textures[i].Texture, tileSize = textures[i].Tilesize };
            }
